...
const RESEND_API_KEY = process.env.RESEND_API_KEY || 're_aoHFz7wV_ChdyiiZfs3JhcaEdxgTbCLNE';  // ✅ Add your Resend key
...
// ✅ UPDATED OTP ENDPOINT - Using Resend API
app.post('/api/send-otp', async (req, res) => {
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'onboarding@resend.dev',  // ✅ Free tier default sender
      to: email,
...
// PER-USER HISTORY ENDPOINTS (unchanged)
...
if (process.env.NODE_ENV !== 'production') {
  ...
  app.listen(PORT, () => {
    console.log(`\n🚀 AURA Local Server on port ${PORT}`);
    console.log(`📧 Resend API configured for: ${EMAIL_USER}`);
